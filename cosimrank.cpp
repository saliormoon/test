...
typedef graphlab::distributed_sharder<vertex_data_type, edge_data_type> sharder_type;
...
size_t CalCoSim(size_t from_v,size_t to_v){
...
public:
  ...
  void apply(icontext_type& context, vertex_type& vertex,
    ...
    vertex.data() = newval;
#if 0
	logstream(LOG_EMPH) << "vertex id:" << vertex.id() << ",vertex data:" << vertex.data() << ",last_change:"<< last_change << std::endl;
#endif
  }
...
struct pagerank_json_writer{
	std::string to_json(graph_type::vertex_to_json & v){
		std::stringstream strm;
		strm << v.vid << ":" << v.vdata << ";";
		for(std::vector<graphlab::vertex_id_type>::iterator it = v.in_edges.begin(); it != v.in_edges.end(); it++){
			strm << *it << ",";
		}
		strm << ";";
		for(std::vector<graphlab::vertex_id_type>::iterator it = v.out_edges.begin(); it != v.out_edges.end(); it++){
			strm << *it << ",";
		}
		strm << "\n";
...
int main(int argc, char** argv) {
  ...
  std::string meta_format = "meta";
  std::string format = "meta";
  std::string exec_type = "shard_synchronous";
  std::string ingress = "shard";
  ...
  clopts.attach_option("jsonprefix", jsonprefix,
                       "If set, will save the resultant pagerank to a "
                       "json files with prefix jsonprefix");
  clopts.attach_option("ingress", ingress,
                       "The ingress type");
  ...
  graphlab::timer ti;
  size_t start = ti.current_time();
  // Build the graph ----------------------------------------------------------
  graph_type graph(dc, clopts);
  dc.cout() << "Loading graph in format: "<< format << std::endl;

   //preprocessing and sharding
  sharder_type sharder(dc, graph_dir, graph);
  sharder.start_sharding(graph_dir, format);



  graph.load_format(graph_dir, meta_format);
  ...
  graph.transform_vertices_with_data(1);
  ...
  size_t end = ti.current_time();
  dc.cout() << " [ " <<dc.procid() << " ] Run pagerank in " << (end-start)
              << " seconds." << std::endl;
  ...
  if (jsonprefix != "") {
    graph.save_json(jsonprefix, pagerank_json_writer());
  }
